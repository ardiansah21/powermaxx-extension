...
- Retry polling worker untuk status transient (kondisi network/server sementara) menggunakan exponential backoff + jitter dan menghasilkan log observability terstruktur (`worker.loop.start`, `worker.claim.empty`, `worker.poll.retry`, `worker.loop.stop`).
